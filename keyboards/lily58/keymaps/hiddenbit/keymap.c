...
#define RAISE MO(_NAVIGATION)
#define LOWER MO(_SPECIAL_CHARS)
...
[_QWERTY] = LAYOUT(
  ...
  KC_LSFT,   KC_Z,    KC_X,     KC_C,     KC_V,     KC_B,     MY_L_ROT,   RGB_TOG,  KC_N,      KC_M,     KC_COMM,  KC_DOT,  KC_SLSH,  KC_RSFT,
                                KC_LCTRL, KC_LALT,  KC_LGUI,  KC_SPC,     KC_ENT,   RAISE,    KC_BSPC,  KC_RGUI
),
...
bool encoder_update_user(uint8_t index, bool clockwise) {
  if (index == 0) { // Encoder on master side
    ...
    if(IS_LAYER_ON(_SPECIAL_CHARS)) { // on Lower layer
      //
      if (clockwise) {
          tap_code(KC_RIGHT);
      } else {
